...
export default function ButtonTextAndIcon({ color, imageSource, action, children, w, h, isActive, isLeft = false }) {
    ...
    return (
        <Pressable disabled={isActive}
            style={ ({ pressed }) => [styles.buttonBasics, styles.textIconButtonBasics, { transform: [{ scale: pressed ? 0.95 : 1 }], backgroundColor: !pressed ? color : colorPressed, marginHorizontal: 5, marginVertical: 20, borderColor: isActive ? "#ACACAC" : customColors.dark}]} onPress={action}>
            {isLeft ?
